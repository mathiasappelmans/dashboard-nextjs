...
* if null means the user is not logged, redirect to login page.
* Else redirect to requested route (HOME_PAGE_URL by default)
*/
	...
	async function middleware(request) {
		...
		if (!request.nextauth.token && request.nextUrl.pathname !== '/login') {
			...
			if (!(request.nextUrl.pathname === '/')) {
				...
				const searchParamsStr = new URLSearchParams({ redirectTo: HOME_PAGE_URL }).toString()
